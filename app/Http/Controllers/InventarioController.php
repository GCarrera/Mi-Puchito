    ...
    public function get_inventorybk(Request $request)//WEB
    {
      ...
      try {
        ...
        if ($request->id != 0) {
          ...
          $productUpdated = Product::where('updated_at', '>', $request->id["updatedProd"])->get();
        ...
        return response()->json(['created' => $inventoryCreated, 'updated' => $inventoryUpdated, 'deleted' => $softdeletes, 'productUpdated' => $productUpdated]);
    ...
    public function all_product_price(Request $request)
	{
		...
		foreach ($request->data as $key => $value) {     
            foreach ($value as $key => $value) {
                return response()->json($value['inventory_id']);
            }       
            $product = Product::where('inventory_id', $precio['inventory_id']);
            if ($product->retail_total_price != $precio['retail_total_price']) {
