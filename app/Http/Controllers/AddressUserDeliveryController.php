...
class AddressUserDeliveryController extends Controller
{
	...
	{
		$user     = auth()->user();
		$rates    = AddressUserDelivery::with('travel_rate')->where('id', $id)->first();
		return $rates;
		//$sectores = TravelRate::where('user_id', $user->id)->get();
	...
	public function update(Request $req, $id)
	{
		...
		*/
			...
			if ($req->forma_delivery == 1 || $req->forma_delivery == 2) {//SI LA OPCION ES ESCRIBIR LA DIRECCION O BUSCARLA
				...
				$address_delivery->user_id = $user_id;*/
					...
					if ($req->forma_delivery == 1 ) {
						...
						$travel = TravelRate::find($req->travel_rate);
						//return $req->travel_rate;
						$travel->sector_id = $req->sector_id;
						$travel->save();
		...
		return back()->with('success', 'Dirección editada correctamente.');
	}
